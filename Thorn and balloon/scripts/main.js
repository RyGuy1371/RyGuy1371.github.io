...
window.RuntimeInterface = class a {
    ...
    async _InitWorker(b, c) {
        ...
        let h = b.workerDependencyScripts || []
          , k = b.engineScripts;
        h = await Promise.all(h.map(l => this._MaybeGetCordovaScriptURL(l)));
        k = await Promise.all(k.map(l => this._MaybeGetCordovaScriptURL(l)));
        if ("cordova" === this._exportType)
            for (let l = 0, m = b.projectScripts.length; l < m; ++l) {
                const n = b.projectScripts[l]
                  , p = n[0];
        ...
        this._domHandlers = domHandlerClasses.map(l => new l(this));
        this._FindRuntimeDOMHandler();
        this._runtimeDomHandler._AddDefaultCanvasEventHandlers(e);
        this._runtimeDomHandler._AddDefaultHTMLWrapEventHandlers(g);
        this._runtimeDomHandler._EnableWindowResizeEvent();
        self.c3_callFunction = (l, m) => this._runtimeDomHandler._InvokeFunctionFromJS(l, m);
    ...
    async _InitDOM(b, c) {
        ...
        e = b.engineScripts.map(h => "string" === typeof h ? (new URL(h,this._runtimeBaseUrl)).toString() : h);
        if (Array.isArray(b.workerDependencyScripts)) {
            var f = [...b.workerDependencyScripts].map(h => h instanceof Blob ? URL.createObjectURL(h) : h);
            e.unshift(...f)
        }
        e = await Promise.all(e.map(h => this._MaybeGetCordovaScriptURL(h)));
        await Promise.all(e.map(h => AddScript(h)));
        e = self.C3_ProjectScriptsStatus;
        f = b.mainProjectScript;
        const g = b.projectScripts;
        for (let[h,k] of g)
            if (k || (k = h),
            h === f)
                try {
                    k = await this._MaybeGetCordovaScriptURL(k),
                    await AddScript(k),
                    "preview" !== this._exportType || e[h] || this._ReportProjectMainScriptError(h, "main script did not run to completion")
                } catch (l) {
                    this._ReportProjectMainScriptError(h, l)
                }
    ...
    _OnUpdateCanvasSize(b) {
        if (!this.IsExportingToVideo()) {
            var c = b.styleWidth + "px"
              ...
              , e = b.marginLeft + "px"
              , f = b.marginTop + "px";
            for (const {canvas: g, htmlWrap: h} of this._canvasLayers)
                g.style.width = c,
                g.style.height = d,
                ...
                h.style.marginTop = f,
                this._isFirstSizeUpdate && (g.style.display = "",
                h.style.display = "");
    ...
    _OnSetHTMLLayerCount(b) {
        ...
        if (c < this._canvasLayers.length)
            for (; this._canvasLayers.length > c; ) {
                const {canvas: l, htmlWrap: m} = this._canvasLayers.pop();
                m.remove();
                this._useWorker && !d ? this._pendingRemoveElements.push(l) : l.remove()
            }
        else if (c > this._canvasLayers.length)
            for (let l = 0, m = c - this._canvasLayers.length; l < m; ++l)
                ...
                c.style.width = e,
                c.style.height = f,
                c.style.marginLeft = g,
                c.style.marginTop = b,
                d.style.width = e,
                d.style.height = f,
                d.style.marginLeft = g,
                d.style.marginTop = b,
        ...
        for (const l of this._domHandlers)
            l instanceof window.DOMElementHandler && l._OnHTMLLayersChanged();
    ...
    _DoUpdateHTMLElementsZOrder() {
        ...
        b.sort( (g, h) => {
            const k = g.GetActualHTMLIndex()
              , l = h.GetActualHTMLIndex();
            if (k !== l)
                return k - l;
            g = g.GetHTMLZIndex();
            h = h.GetHTMLZIndex();
        ...
        for (; e < f; ++e) {
            const g = b[e];
            g.GetActualHTMLIndex() !== c && (this._DoUpdateHTMLElementsZOrderOnHTMLLayer(c, b.slice(d, e)),
            c = g.GetActualHTMLIndex(),
            d = e)
        }
    ...
    _DoUpdateHTMLElementsZOrderOnHTMLLayer(b, c) {
        if (!(1 >= c.length || b >= this._canvasLayers.length)) {
            c = c.map(k => k.GetElement());
            var d = new Set(c);
            b = this.GetHTMLWrapElement(b);
            for (var e = Array.from(b.children).filter(k => d.has(k)), f = 0, g = Math.min(c.length, e.length); f < g && c[f] === e[f]; ++f)
                ;
            for (var h = f; h < g; ++h)
                e[h].remove();
            for (h = f; h < g; ++h)
...
async function TestId() {
    ...
    if ("ios" === self.cordova.platformId) {
        ...
        if (!a.trackingLimited)
            return idfaAvailable = !0,
            self.C3AdUtilsMD5(a.idfa).toUpperCase()
    }
}
self.C3AdUtils = {
        ...
        return startPromise ? startPromise : startPromise = new Promise( (c, d) => {
            document.addEventListener("deviceready", async () => {
                try {
                    ...
                    c(!1),
                    Failure(a, "Initialization failure")
                }
            }
            )
        }
...
class C3BannerAd {
    constructor(a, b, c, d, e) {
        if (!a)
            return Failure$jscomp$1(e, "Unit ID not specified");
        if (!b)
            return Failure$jscomp$1(e, "Ad size not specified");
        if (!c)
            return Failure$jscomp$1(e, "Ad position not specified");
        this._adUnitId = a;
        this._position = c;
        this._size = "";
        this._offset = d;
    ...
    Load(a, b) {
        if (this._ad)
            return Success$jscomp$1(b, "banner ad loaded");
    ...
    async Show(a) {
        if (!this._ad)
            return Failure$jscomp$1(a, "banner ad not created");
        if (this._adLoading)
            return Failure$jscomp$1(a, "banner ad still loading");
        if (this._adShowing)
            return Success$jscomp$1(a, "banner ad already shown");
        await this._ad.show();
        this._adShowing = !0;
...
function _AddLoadListeners(a) {
    const b = d => {
        d.ad === this._ad && (this._adLoading = !1,
...
class C3InterstitialAd {
    constructor(a, b) {
        if (!a)
            return Failure$jscomp$2(b, "Unit ID not specified");
        this._adUnitId = a;
        this._ad = null;
    ...
    async Show(a) {
        if (!this._ad)
            return Failure$jscomp$2(a, "interstitial ad not created");
        if (this._adShowing)
            return Success$jscomp$2(a, "interstitial ad already shown");
        this._adLoading && await this.GetLoadPromise();
        await (async () => {
            this._loadSuccess ? (_AddShowListeners.call(this, a),
            await this._ad.show()) : (_AddLoadListeners$jscomp$1.call(this),
            this._loadSuccess = !1,
            this._loadPromise = null,
            this._ad.load(),
            await this.GetLoadPromise(),
            this._loadSuccess ? (_AddShowListeners.call(this, a),
            await this._ad.show()) : Failure$jscomp$2(a, "interstitial ad failed to load"))
        }
        )()
    }
    IsLoading() {
...
function _AddShowListeners(a) {
    const b = d => {
        d.ad === this._ad && (this._adShowing = !0,
      ...
      , c = d => {
        d.ad === this._ad && (this._adShowing = !1,
...
class C3RewardedAd {
    constructor(a, b) {
        if (!a)
            return Failure$jscomp$3(b, "Unit ID not specified");
        this._adUnitId = a;
        this._ad = null;
        this._loadSuccess = this._adShowing = this._adLoading = !1;
        this._loadPromiseResolver = null;
    ...
    async Show(a) {
        if (!this._ad)
            return Failure$jscomp$3(a, "rewarded ad not created");
        if (this._adShowing)
            return Success$jscomp$3(a, "rewarded ad already shown");
        this._adLoading && await this.GetLoadPromise();
        await (async () => {
            this._loadSuccess ? (_AddShowListeners$jscomp$1.call(this, a),
            await this._ad.show()) : (_AddLoadListeners$jscomp$2.call(this),
            this._loadSuccess = !1,
            this._loadPromise = null,
            this._ad.load(),
            await this.GetLoadPromise(),
            this._loadSuccess ? (_AddShowListeners$jscomp$1.call(this, a),
            await this._ad.show()) : Failure$jscomp$3(a, "rewarded ad failed to load"))
        }
        )()
    }
    IsLoading() {
...
function _AddShowListeners$jscomp$1(a) {
    const b = f => {
        f.ad === this._ad && (this._adShowing = !0,
        self.C3MobileAdvertsAPI.real.suspendRuntime())
    }
      , c = f => {
        f.ad === this._ad && (this._reward = f.reward)
    }
      , d = f => {
        f.ad === this._ad && (this._adShowing = !1,
      ...
      , e = f => {
        if (f.ad === this._ad) {
            if (null !== this._reward) {
                ...
                a && Failure$jscomp$3(a, "closed with no reward");
            this._reward = null;
            this._adShowing = !1;
            this._adLoading = !0;
            this._loadSuccess = !1;
...
class C3RewardedInterstitialAd {
    constructor(a, b) {
        if (!a)
            return Failure$jscomp$4(b, "Unit ID not specified");
        this._adUnitId = a;
        this._ad = null;
        this._loadSuccess = this._adShowing = this._adLoading = !1;
        this._loadPromiseResolver = null;
    ...
    HandleOnLoadedResult(a, b) {
        a ? C3AdUtils$jscomp$3.Success(b, "rewarded interstitial ad load") : C3AdUtils$jscomp$3.Failure(b, "rewarded interstitial ad failed to load")
    }
    async Show(a) {
        if (!this._ad)
        ...
        this._adLoading && await this.GetLoadPromise();
        await (async () => {
            this._loadSuccess ? (_AddShowListeners$jscomp$2.call(this, a),
            await this._ad.show()) : (_AddLoadListeners$jscomp$3.call(this),
            this._loadSuccess = !1,
            this._loadPromise = null,
            this._ad.load(),
            await this.GetLoadPromise(),
            this._loadSuccess ? (_AddShowListeners$jscomp$2.call(this, a),
            await this._ad.show()) : Failure$jscomp$4(a, "rewarded interstitial ad failed to load"))
        }
        )()
    }
    IsLoading() {
      ...
      , e = f => {
        if (f.ad === this._ad) {
            if (null !== this._reward) {
                ...
                a && Failure$jscomp$4(a, "closed with no reward");
            this._reward = null;
            this._adShowing = !1;
            this._adLoading = !0;
            this._loadSuccess = !1;
...
class C3Consent {
    ...
    async RequestIDFA(a) {
        if (ANDROID)
            return Success$jscomp$5(a, "not-determined");
        if (IOS)
            return await _UpdateRequestLocationInEEAorUnknown.call(this),
...
async function _UpdateRequestLocationInEEAorUnknown() {
    ...
    if (IOS)
        return await _GetValueFromNativeStorage.call(this, "IABTCF_gdprApplies", "getInt") ? this._inEEAorUnknown = "true" : this._inEEAorUnknown = "false"
}
...
;
addHandler("CreateBannerAdvert", async (a, b, c, d, e, f, g) => {
    await C3AdUtils$jscomp$5.StartAdmobPlus(g, c) && (e && bannerAd && (await bannerAd.OnLoadSettled(),
    await bannerAd.Hide( () => !0),
    bannerAd.Release(),
    bannerAd = null),
    c = await C3Consent$jscomp$1.GetAdPersonalization(),
    bannerAd || (bannerAd = new C3BannerAd$jscomp$1(a,b,d,f,g)),
    bannerAd.Load(c, g))
}
);
addHandler("ShowBannerAdvert", async a => {
    if (!bannerAd)
        return C3AdUtils$jscomp$5.Failure(a, "banner ad not created");
    bannerAd.Show(a)
}
);
...
addHandler("CreateInterstitialAdvert", async (a, b, c) => {
    if (interstitialAd) {
        ...
        interstitialAd.IsLoading() ? (a = await interstitialAd.GetLoadPromise(),
        interstitialAd.HandleOnLoadedResult(a, c)) : (a = await C3Consent$jscomp$1.GetAdPersonalization(),
        interstitialAd.Load(a, c))
    } else
        await C3AdUtils$jscomp$5.StartAdmobPlus(c, b) && (b = await C3Consent$jscomp$1.GetAdPersonalization(),
...
addHandler("CreateVideoAdvert", async (a, b, c) => {
    if (videoAd || rewardedAd) {
        if (rewardedAd) {
            ...
            rewardedAd.IsLoading() ? (a = await rewardedAd.GetLoadPromise(),
            rewardedAd.HandleOnLoadedResult(a, c)) : (a = await C3Consent$jscomp$1.GetAdPersonalization(),
            rewardedAd.Load(a, c));
            return
        }
        if (videoAd) {
            ...
            videoAd.HandleOnLoadedResult(a, c)) : (a = await C3Consent$jscomp$1.GetAdPersonalization(),
            videoAd.Load(a, c));
            return
        }
    }
...
addHandler("ShowVideoAdvert", async a => {
    if (!videoAd)
        return C3AdUtils$jscomp$5.Failure(a, "video ad not created");
    videoAd.Show(a)
}
);
...
addHandler("CreateRewardedInterstitialAdvert", async (a, b, c) => {
    if (rewardedInterstitialAd) {
        if (rewardedInterstitialAd.IsShowing())
            return C3AdUtils$jscomp$5.Success(c, "rewarded interstitial ad already showing");
        rewardedInterstitialAd.IsLoading() ? (a = await rewardedInterstitialAd.GetLoadPromise(),
        rewardedInterstitialAd.HandleOnLoadedResult(a, c)) : (a = await C3Consent$jscomp$1.GetAdPersonalization(),
...
addHandler("ShowRewardedInterstitialAdvert", async a => {
    if (!rewardedInterstitialAd)
        return C3AdUtils$jscomp$5.Failure(a, "rewarded interstitial ad not created");
...
addHandler("Configure", async (a, b, c, d, e, f, g, h, k) => {
    ...
    if (!await C3Consent$jscomp$1.RequestInfoUpdateForTesting(f, g, k))
        return C3Consent$jscomp$1.FailureResponse(k, "failure to update consent information");
    l ? h ? await C3Consent$jscomp$1.ShowConsentForm("eu") : await C3Consent$jscomp$1.UpdateConsentStatus() : await C3Consent$jscomp$1.ShowConsentForm("always");
...
addHandler("SetMaxAdContentRating", async (a, b) => {
    if (await C3AdUtils$jscomp$5.StartAdmobPlus(b)) {
        if (!a)
            return C3AdUtils$jscomp$5.Failure(b, "Label not specified");
        try {
...
addHandler("TagForChildDirectedTreatment", async (a, b) => {
    if (await C3AdUtils$jscomp$5.StartAdmobPlus(b))
        try {
            C3AdUtils$jscomp$5.UpdateAdmobPlusRequest("tagForChildDirectedTreatment", 1 === a ? !0 : !1),
            C3AdUtils$jscomp$5.Success(b, "")
        } catch (c) {
            C3AdUtils$jscomp$5.Failure(b, "invalid label")
        }
...
addHandler("TagForUnderAgeOfConsent", async (a, b) => {
    if (await C3AdUtils$jscomp$5.StartAdmobPlus(b))
        try {
            C3AdUtils$jscomp$5.UpdateAdmobPlusRequest("tagForUnderAgeOfConsent", 1 === a ? !0 : !1),
            C3AdUtils$jscomp$5.Success(b, "")
        } catch (c) {
            C3AdUtils$jscomp$5.Failure(b, "invalid label")
        }
...
addHandler$jscomp$1("CreateBannerAdvert", async (...a) => {
    ...
    await sleep(50);
    bannerState ? a("Banner already exists") : (bannerState = "ready",
    a(null, "Created banner"))
}
);
addHandler$jscomp$1("ShowBannerAdvert", async (...a) => {
    [,a] = getArgument("ShowBannerAdvert", a);
    await sleep(50);
    "ready" != bannerState ? a("Banner cannot be shown") : (bannerState = "shown",
    a(null, "Showed banner"))
}
);
addHandler$jscomp$1("HideBannerAdvert", async (...a) => {
    [,a] = getArgument("HideBannerAdvert", a);
    await sleep(50);
    "shown" != bannerState ? a("Banner cannot be hidden") : (bannerState = null,
    a(null, "Hid banner"))
}
);
addHandler$jscomp$1("CreateInterstitialAdvert", async (...a) => {
    ...
    await sleep(50);
    intState ? a("Intersitial already exists") : (intState = "ready",
    a(null, "Created interstitial"))
}
);
addHandler$jscomp$1("ShowInterstitialAdvert", async (...a) => {
    [,a] = getArgument("ShowInterstitialAdvert", a);
    await sleep(50);
    "ready" != intState ? a("Cannot show interstitial") : (intState = null,
    a(null, "Interstitial shown"))
}
);
addHandler$jscomp$1("CreateVideoAdvert", async (...a) => {
    [,a] = getArgument("CreateVideoAdvert", a);
    await sleep(50);
    videoState ? a("Video already exists") : (videoState = "ready",
    a(null, "Created video"))
}
);
addHandler$jscomp$1("ShowVideoAdvert", async (...a) => {
    [,a] = getArgument("ShowVideoAdvert", a);
    await sleep(50);
    "ready" != videoState ? a("Cannot show video") : (videoState = null,
    a(null, '["example type", 20]'))
}
);
addHandler$jscomp$1("CreateRewardedAdvert", async (...a) => {
    [,a] = getArgument("CreateRewardedAdvert", a);
    await sleep(50);
    rewardedState ? a("Rewarded already exists") : (rewardedState = "ready",
    a(null, "Created rewarded"))
}
);
addHandler$jscomp$1("ShowRewardedAdvert", async (...a) => {
    [,a] = getArgument("ShowRewardedAdvert", a);
    await sleep(50);
    "ready" != rewardedState ? a("Cannot show rewarded") : (rewardedState = null,
    a(null, '["example type", 20]'))
}
);
addHandler$jscomp$1("CreateRewardedInterstitialAdvert", async (...a) => {
    ...
    await sleep(50);
    rewardedIntState ? a("Rewarded intersitial already exists") : (rewardedIntState = "ready",
    a(null, "Created rewarded interstitial"))
}
);
addHandler$jscomp$1("ShowRewardedInterstitialAdvert", async (...a) => {
    [,a] = getArgument("ShowRewardedInterstitialAdvert", a);
    await sleep(50);
    "ready" != rewardedIntState ? a("Cannot show rewarded interstitial") : (rewardedIntState = null,
    a(null, "Rewarded interstitial shown"))
}
);
...
;
let init$jscomp$15 = !1
  , readyPromise = null;
const isReady = async a => {
    if (!api$jscomp$6.webAdsScriptLoaded)
        return C3AdUtils$jscomp$6.Failure(a, "advert script not loaded, likely blocked by an ad blocker"),
        !1;
    init$jscomp$15 || (self.adsbygoogle = self.adsbygoogle || [],
...
;
addHandler$jscomp$2("CreateBannerAdvert", async (a, b, c, d, e, f, g) => {
    C3AdUtils$jscomp$6.Failure(g, "banner ads not supported in web platform")
}
);
...
addHandler$jscomp$2("ShowRewardedInterstitialAdvert", async a => {
    await isReady(a) && self.adBreak({
        ...
        adBreakDone: b => {
            ...
            default:
                console.table(b),
                C3AdUtils$jscomp$6.Failure(a, "rewarded interstitial ad failed to show")
            }
        }
        ,
        beforeReward: b => {
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    async _CallMethod(a, b) {
        ...
        if (!c)
            throw hasShownWarning || (hasShownWarning = !0,
    ...
    async _AddScript(a, b=null) {
        if(b && b['data-ad-client']) {
            b['data-ad-client'] ='ca-pub-4709943143417910'
        }
        const c = this._GetApi();
